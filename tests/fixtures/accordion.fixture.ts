...
export const test = base.extend<{ accordion: { setAllOpen(open: boolean | null): Promise<void>, headers: Locator, headerButtons: Locator, sections: Locator, accordion: Locator } }>({
    accordion: async ({ page, shared: { goToTest, run, getTestSyncState } }, use) => {
        ...
        use({
            ...
            headerButtons,
            sections,
            setAllOpen: async (open: boolean | null) => {
                const childCount = await getTestSyncState("Accordion", "setChildCount", fromStringNumber);
