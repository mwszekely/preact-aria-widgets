            ...
            test.describe(disabled ? (`Don't fire when ${(disabledType == "soft" ? " soft-" : "")}disabled`) : "Fire when not disabled", () => {
                for (const buttonType of buttonTypes) {
                    for (const eventType of eventTypes) {
                        ...
                        test(`${eventType} on a ${buttonType} ${!disabled ? `fires under normal conditions` : `doesn't fire when ${disabledType == "soft" ? "soft-" : ""}disabled`}`, async ({ page, button: { getCounter, resetCounter, buttons }, shared: { install, run, locator } }) => {
...
test.describe("Roles are correct", () => {
    ...
    for (const buttonType of buttonTypes) {
        ...
        test(`${capitalize(buttonType)}`, async ({ page, button: { getCounter, resetCounter, buttons }, shared: { install, run, locator } }) => {
            //for (const [buttonType, button] of Object.entries(buttons)) {
            const button = buttons[buttonType];
            resetCounter();
